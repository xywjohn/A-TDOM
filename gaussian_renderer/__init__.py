...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, Render_Mask=None, TDOM=False):
    """
    Render the scene. 
    ...
    if len(Render_Mask.shape) == 1:
        Use_Render_Mask = False
        # Render_Mask = torch.ones(1, int(viewpoint_camera.image_height), int(viewpoint_camera.image_width), dtype=torch.bool).cuda()
        # Render_Mask = torch.ones(1, dtype=torch.bool).cuda()
    else:
        Use_Render_Mask = True
    ...
    raster_settings = GaussianRasterizationSettings(
        ...
        Use_Render_Mask=Use_Render_Mask,
        TDOM=TDOM
    )
