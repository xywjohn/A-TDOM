...
class Scene:
    ...
    def __init__(self, args, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0], Do_Get_Tri_Mask=False, No_Key_Region=False):
        ...
        self.basic_pcd = scene_info.point_cloud
    ...
    def save(self, iteration, mask=None):
        ...
        if mask == None:
            ...
            self.gaussians.save_ply(os.path.join(point_cloud_path, "point_cloud_part.ply"), mask)

    def setTrainCameras(self, newTrainCameras, scale=1.0):
        del self.train_cameras[scale]
        self.train_cameras[scale] = newTrainCameras
...
class SingleImage_Scene:

    gaussians: GaussianModel

    def __init__(self, args, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0], Do_Get_Tri_Mask=False, Image_Name="", points_per_triangle=4):
        """b
        :param path: Path to colmap scene main folder.
        """
        self.model_path = args.model_path
        ...
        if os.path.exists(os.path.join(args.source_path, "sparse")):
            scene_info = sceneLoadTypeCallbacks["Single_Image"](args.source_path, args.images, args.eval, Do_Get_Tri_Mask=Do_Get_Tri_Mask, Image_Name=Image_Name, points_per_triangle=points_per_triangle)
            self.scene_info = scene_info
            self.scene_info_traincam = scene_info.train_cameras
        elif os.path.exists(os.path.join(args.source_path, "transforms_train.json")):
            print("Found transforms_train.json file, assuming Blender data set!")
