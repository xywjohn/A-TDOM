...
class GaussianModel:
    ...
    # 如果仅训练当前新传入的影像
    def SingleImage_Update_Lr(self, ImagesAlreadyBeTrainedIterations, args, viewpoint_cam, IterationPerImage):
        delay_rate = 1.0
        if (ImagesAlreadyBeTrainedIterations[viewpoint_cam.image_name] >= IterationPerImage):
            t = 1.0
        else:
            t = np.clip(ImagesAlreadyBeTrainedIterations[viewpoint_cam.image_name] / IterationPerImage, 0, 1)

        log_lerp = np.exp(np.log(args.position_lr_init * self.spatial_lr_scale) * (1 - t) + np.log(args.position_lr_final * self.spatial_lr_scale) * t)
        log_lerp = delay_rate * log_lerp

        # 更新优化器中的参数
        for param_group in self.optimizer.param_groups:
            if param_group["name"] == "xyz":
                param_group['lr'] = log_lerp
    ...
    def capture(self, mask=None):
        if mask == None:
            ...
            Part_optimizer = copy.deepcopy(self.optimizer)
            for group in Part_optimizer.param_groups:
                stored_state = Part_optimizer.state.get(group['params'][0], None)
                if stored_state is not None:
                    stored_state["exp_avg"] = stored_state["exp_avg"][mask]
                    stored_state["exp_avg_sq"] = stored_state["exp_avg_sq"][mask]

                    del Part_optimizer.state[group['params'][0]]
                    group["params"][0] = nn.Parameter((group["params"][0][mask].requires_grad_(True)))
                    Part_optimizer.state[group['params'][0]] = stored_state
                else:
                    group["params"][0] = nn.Parameter(group["params"][0][mask].requires_grad_(True))
            return (
                self.active_sh_degree,
                ...
                self._rotation[mask],
                self._opacity[mask],
                self.max_radii2D[mask],
                self.xyz_gradient_accum[mask],
                self.denom[mask],
                Part_optimizer.state_dict(),
                self.spatial_lr_scale,
            )
    ...
    def save_ply(self, path, mask=None):
        ...
        xyz = self._xyz.detach().cpu().numpy() if mask == None else self._xyz[mask].detach().cpu().numpy()
        normals = np.zeros_like(xyz)
        f_dc = self._features_dc.detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy() if mask == None else self._features_dc[mask].detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
        f_rest = self._features_rest.detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy() if mask == None else self._features_rest[mask].detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
        opacities = self._opacity.detach().cpu().numpy() if mask == None else self._opacity[mask].detach().cpu().numpy()
        scale = self._scaling.detach().cpu().numpy() if mask == None else self._scaling[mask].detach().cpu().numpy()
        rotation = self._rotation.detach().cpu().numpy() if mask == None else self._rotation[mask].detach().cpu().numpy()
    ...
    def reset_opacity(self, Image_Visibility=None):
        opacities_new = self.inverse_opacity_activation(torch.min(self.get_opacity, torch.ones_like(self.get_opacity)*0.01))
        if Image_Visibility is not None:
            Visibility_List = list(Image_Visibility.values())
            GaussiansVisibleMask = torch.stack(Visibility_List, dim=0)
            GaussiansVisibleMask = torch.any(GaussiansVisibleMask, dim=0)
            opacities_new[~GaussiansVisibleMask] = self.get_opacity[~GaussiansVisibleMask]
